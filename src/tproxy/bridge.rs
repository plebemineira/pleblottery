...
pub struct Bridge {
    ...
    tx_status: status::Sender<'static>,
...
impl Bridge {
    ...
    pub fn on_new_sv1_connection(
        ...
        match self.channel_factory.new_extended_channel(0, hash_rate, 0) {
                    ...
                    match message {
                        Mining::OpenExtendedMiningChannelSuccess(success) => {
                            ...
                            self.target
                                ...
                                .map_err(|e| PoisonLock(e.to_string()))?;
            ...
            Err(_) => {
                return Err(LotteryError::SubprotocolMining(
                    "Bridge: failed to open new extended channel".to_string(),
                ))
            }
        };
        Err(LotteryError::SubprotocolMining(
    ...
    fn translate_submit(
        ...
        let last_version = self
            ...
            .ok_or(LotteryError::RolesSv2Logic(RolesLogicError::NoValidJob))?;
        let version = match (sv1_submit.version_bits, version_rolling_mask) {
            ...
            _ => return Err(LotteryError::V1Protocol(sv1_api::error::Error::InvalidSubmission)),
        };
    ...
    async fn handle_new_prev_hash_(
        ...
        while let Some(job) = future_jobs.pop() {
            if job.job_id == sv2_set_new_prev_hash.job_id {
                ...
                let notify = tproxy::next_mining_notify::create_notify(
    ...
    async fn handle_new_extended_mining_job_(
        ...
        if sv2_new_extended_mining_job.is_future() {
            ...
            let last_p_hash = last_p_hash_option.ok_or(LotteryError::RolesSv2Logic(
