...
impl Upstream {
        ...
        task::spawn(async move {
            loop {
                ...
                let message_type =
                    incoming
                        .get_header()
                        .ok_or(super::super::error::LotteryError::FramingSv2(
                    ...
                    Ok(SendTo::Respond(message_for_upstream)) => {
                        ...
                        handle_result!(
                            ...
                            tx_frame.send(frame).await.map_err(|e| {
                                super::super::error::LotteryError::ChannelErrorSender(
    ...
    fn get_job_id(
        self_: &Arc<Mutex<Self>>,
    ) -> Result<Result<u32, super::super::error::LotteryError<'static>>, super::super::error::LotteryError<'static>>
    {
        self_
            .safe_lock(|s| {
                if s.is_work_selection_enabled() {
                    s.last_job_id
                        .ok_or(super::super::error::LotteryError::RolesSv2Logic(
                            RolesLogicError::NoValidTranslatorJob,
                        ))
                } else {
                    s.job_id.ok_or(super::super::error::LotteryError::RolesSv2Logic(
